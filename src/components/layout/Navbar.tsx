...
export default function Navbar({ fixedTop }: { fixedTop?: boolean }) {
  ...
  return (
    <nav className={(fixedTop ? "navbar navbar-offset sticky top-0" : "navbar") + " z-50 py-2 transition-all duration-500 border-b border-transparent bg-white/80 backdrop-blur-md shadow-sm"}>
      <div className="navbar-container max-w-7xl mx-auto px-4" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '16px' }}>
          ...
          {[
            ...
            <li key={item.href}>
              <Link 
                ...
                style={{ fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#475569', paddingBottom: '4px' }}
              >
                {item.label}
                <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-primary transition-all group-hover/item:w-full"></span>
              </Link>
            </li>
          ))}
        </ul>
